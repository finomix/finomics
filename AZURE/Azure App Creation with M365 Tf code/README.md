# Azure App Creation with Microsoft 365 (M365) - Terraform
...
- Microsoft Graph API Permission Assignments (M365)
...
The key difference from the base Azure App Creation module is the addition of **Microsoft Graph (M365) API permissions**, which are assigned directly to the Service Principal using app role assignments.
...
```
├── main.tf            # Core resources: App, SP, secret, Graph permissions, role assignments
...
> These permissions are assigned via `azuread_app_role_assignment` using the Microsoft Graph service principal.
...
| State Key             | `prod-1/terraform.tfstate`   |
...
```
# The client_secret is marked as sensitive, so it will not be displayed directly in the terminal.
To view it, run:
...
- Ensure the account running Terraform has sufficient privileges to create app registrations, assign API permissions, and assign roles at the subscription level.
